...
此前每次 LLM 调用都是冷启动：只发 `[system, user]` 两条，模型看不到自己上一步
决定了什么、为什么那么定坐标系，于是每步重新猜。本模块让一个 session 内的
start_plan / next_step / evaluate_step 共享一条持续增长的消息流。

**记什么**：助手的原始输出（它自己的决策与理由，这正是缺失的连续性）＋
每回合精简的结果说明。**不记什么**：每轮 user_message 里的文档状态快照——
那是快照不是增量，累积它既浪费 token 又会让旧状态和新状态互相矛盾。
当前轮的完整快照只作为最后一条 user 消息发出去。
"""
